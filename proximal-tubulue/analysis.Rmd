...
```{r}
k562 = load_k562("metadata/k562.counts")
k562_pseudo = load_k562("metadata/k562-pseudo-60million.counts")
k562_quasi = load_k562("metadata/k562-quasi-60million.counts")
...
We'll look at just the top 150 cells with the most counts in each sample, so we
can compare more easily to the K562 cells.
...
k562 = top_n_cell(k562)
k562_pseudo = top_n_cell(k562_pseudo)
k562_quasi = top_n_cell(k562_quasi)
h1 = top_n_cell(h1)
h2 = top_n_cell(h2)
h3 = top_n_cell(h3)
```
...
k562_pseudo_detected = data.frame(cell=colnames(k562_pseudo),
                                  total=colSums(k562_pseudo),
                                  detected=colSums(k562_pseudo > 0),
                                  sample="k562_pseudo")
k562_quasi_detected = data.frame(cell=colnames(k562_quasi), total=colSums(k562_quasi),
                           detected=colSums(k562_quasi > 0), sample="k562_quasi")
...
detected = rbind(k562_detected, h1_detected, h2_detected, h3_detected,
                 k562_pseudo_detected, k562_quasi_detected)
...
k562_pseudo_detected = data.frame(gene=rownames(k562_pseudo),
                                  total=rowSums(k562_pseudo),
                                  detected=rowSums(k562_pseudo > 0),
                                  sample="k562_pseudo")
k562_quasi_detected = data.frame(gene=rownames(k562_quasi), total=rowSums(k562_quasi),
                           detected=rowSums(k562_quasi > 0), sample="k562_quasi")
...
detected = rbind(k562_detected, h1_detected, h2_detected, h3_detected,
                 k562_quasi_detected, k562_pseudo_detected)
...
### K562 pseudo
```{r}
housekeeping_heatmap(k562_pseudo, human_hk)
```

### K562 quasi
```{r}
housekeeping_heatmap(k562_quasi, human_hk)
```
...
and has the dip, so it isn't due to the depth of sequencing. Next step is
to look at the housekeeping genes.
